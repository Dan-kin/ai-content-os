...
class OutputTest(unittest.TestCase):
    ...
    def test_render_prompt_pack_contains_cn_and_en_prompts(self):
        ...
        self.assertIn('### 直接复制给出图 AI', pack)
        self.assertIn('请根据下面要求生成一张公众号配图。', pack)
        self.assertIn('图片比例：16:9', pack)
        self.assertIn('用途：用一张图建立文章主题', pack)
        self.assertIn('避免：不要文字，不要赛博朋克，不要夸张表情', pack)
